...
async function embedResources(cssText, baseUrl, options) {
  ...
  return urls.reduce((deferred, url) => deferred.then((css5) => embed(css5, url, baseUrl, options)), Promise.resolve(filteredCSSText));
}
...
async function captureAndUploadScreenshot(annotation, endpoint) {
  try {
    ...
    const blob = await new Promise(
      (resolve) => canvas.toBlob(resolve, "image/jpeg", 0.75)
    );
...
function drawHighlightOnCanvas(canvas, annotation) {
  const ctx = canvas.getContext("2d");
  if (!ctx) return;
  const scrollY = window.scrollY;
  const boxes = annotation.elementBoundingBoxes?.length ? annotation.elementBoundingBoxes : annotation.boundingBox ? [annotation.boundingBox] : [];
  ctx.save();
...
function PageFeedbackToolbarCSS({
  ...
  const hideToolbarTemporarily = useCallback2(() => {
